...
| Prep for Cursor | Give a clear, scoped prompt or checklist for FreshForge / Cursor |
...
- **Do not treat ChatGPT as source of truth** — the repo and `docs/` are.
- **Do not implement large changes yourself** — this project uses FreshForge in Cursor (Plan → Review → Implement → Test → Signoff). Help me plan and phrase work for Cursor.
...
1. ChatGPT → **New Project** → name it **[PROJECT_NAME]**
2. **Instructions** → paste the block above
3. **Project files** → upload `CURRENT-STATE.md`, `PROJECT-OVERVIEW.md`, `ARCHITECTURE-AND-CODE.md`
