...
| Install dry-run default | `node scripts/install-freshforge.mjs --target ./tmp-install-test --dry-run` | 0 | Pass |
...
| Export excludes `docs/freshforge-development/` | Pass |
...
| `--include-readme` adds `FRESHFORGE_README.md` on install | Pass |
...
- `dist/` added to `.gitignore`; export output verified locally at `dist/freshforge-starter/`.
