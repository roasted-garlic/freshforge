...
Reorganized starter-facing documentation into subfolders under `docs/` and moved FreshForge-only installation, distribution, packaging, and starter-surface docs into `docs/freshforge-development/distribution/`. Updated references across rules, agents, skills, scripts, and validation. Default export/install continues to ship `AGENTS.md`, `.cursor/`, and `docs/` only.
...
**FreshForge-only:** `INSTALLATION.md`, `DISTRIBUTION.md`, `PACKAGING.md`, `STARTER_SURFACE.md` → `docs/freshforge-development/distribution/`.
...
| `node bin/freshforge.mjs install --dry-run` | exit 0 — correct roots and conflicts in dev repo |
...
**Excluded:** `docs/freshforge-development/`, legacy flat paths (`docs/INSTALLATION.md`, etc.)
...
The distributed starter now ships only target-project workflow documentation. FreshForge operator docs remain in the development repo under `docs/freshforge-development/` and are excluded from default install/export.
