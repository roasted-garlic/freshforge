# Plan: Branding Clarification — FreshForge Canonical Naming
...
| Related | docs/freshforge-development/reviews/branding-clarification-review.md |

---

## Goal

Standardize the reusable Cursor workflow starter under the canonical name **FreshForge** and remove confusing **BuildPilot** / **BuildPilot-FreshForge** dual-naming. Docs must clearly describe this repository as a workflow starter kit, not an application.

## Background

The starter was introduced with overlapping names (FreshForge as working name, BuildPilot as folder/repo label). Three explicit `BuildPilot` references remain in `README.md`, `AGENTS.md`, and `docs/WORKFLOWS.md`. Canonical naming reduces agent and user confusion when copying the starter into other repos.

## Scope

### In Scope
- Replace `BuildPilot` references with `FreshForge` where they describe the current project
...
- Clarify template docs describe *target applications*; FreshForge itself is the workflow starter
...
- Changing command aliases (e.g. `Continue FreshForge` stays)
...
- `docs/project/PROJECT_BRIEF.md` — FreshForge starter context note
- `docs/architecture/ARCHITECTURE.md` — FreshForge starter context note
...
1. Update the three files with explicit `BuildPilot` strings to use `FreshForge` only
2. Add **Naming Standard** sections to README.md and AGENTS.md (canonical name, historical note for BuildPilot folder name)
3. Add brief intro notes to project-specific template docs clarifying they apply to *applications built with FreshForge*, not the starter itself
...
- [x] Confirm AGENTS.md uses FreshForge consistently
...
## Approval
- Review doc: docs/freshforge-development/reviews/branding-clarification-review.md
- Verdict: pending
