...
Add `bin/appforge.mjs` so users can run `npx github:roasted-garlic/appforge install` without clone-temp-delete.

## Approach

1. Extract `runInstall` / `runExport` into `scripts/lib/`.
2. Thin wrappers in existing install/export scripts.
3. Add `bin/appforge.mjs` routing install, export, validate.
