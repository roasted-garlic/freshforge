...
function printUsage() {
  console.log(`Usage: node scripts/install-appforge.mjs [options]
...
async function main() {
  ...
  if (summary.conflicts.length > 0 && !options.force) {
    ...
    printSummary(summary, { ...options, target: targetRoot }, getTopLevelRoots(actionable));
    process.exit(1);
  }
  ...
  for (const item of toWrite) {
    await writeItem(item, SOURCE_ROOT, targetRoot, options.dryRun);
  }
